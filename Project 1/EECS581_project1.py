...
def place_ships(board, ship_list):
    ...
    for ship_size in ship_list: # For each ship size
        ...
        while True:
            start = block_till_valid("Enter start position (e.g., A1): ", "Please enter a valid value")
            ...
            if ship_size > 1:   # If the ship size is 1, automatically set orientation and direction
                ...
                if orientation == "H":
                    ...
                    while direction not in ["R", "L"]:
                        print("Invalid direction for horizontal orientation. Please enter 'R' for right or 'L' for left.")
                        direction = input(
                            "Enter direction (R for right, L for left): "
                        ).upper()
                elif orientation == "V":
                    ...
                    while direction not in ["D", "U"]:
                        print("Invalid direction for vertical orientation. Please enter 'D' for down or 'U' for up.")
                        direction = input(
                            "Enter direction (D for down, U for up): "
                        ).upper()
            else:
            ...
            if is_valid_position(start) and valid_ship_placement(board, ship_size, orientation, direction, start):
                ...
                print("Invalid input or placement. Try again.") # Handle invalid placement
...
def battleship_game():
    ...
    _block_till_valid = lambda: block_till_valid("Enter position to fire (e.g., A1): ", "Invalid value, please try again.")
    while True:
        if player_turn == 1:
            print("\n\n\n\n\nPlayer 1's turn!")
            print("\nYour board:")
